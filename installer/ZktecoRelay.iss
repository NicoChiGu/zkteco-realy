...
#if DirExists(SourceDir + "\dll\" + Architecture)
  #define DllSourceDir SourceDir + "\dll\" + Architecture
#elif DirExists(SourceDir + "\dll")
  #define DllSourceDir SourceDir + "\dll"
#elif DirExists(SourceDir + "\..\dll\" + Architecture)
  #define DllSourceDir SourceDir + "\..\dll\" + Architecture
#else
  #define DllSourceDir "..\dll\" + Architecture
#endif
...
[Code]
procedure DeleteSdkDlls(const TargetDir: String);
var
  Dir: String;
begin
  Dir := AddBackslash(TargetDir);
  DeleteFile(Dir + 'zkemkeeper.dll');
  DeleteFile(Dir + 'zkemsdk.dll');
  DeleteFile(Dir + 'zkemsdkutils.dll');
  DeleteFile(Dir + 'commpro.dll');
  DeleteFile(Dir + 'comms.dll');
  DeleteFile(Dir + 'tcpcomm.dll');
  DeleteFile(Dir + 'usbcomm.dll');
  DeleteFile(Dir + 'usbstd.dll');
  DeleteFile(Dir + 'rscagent.dll');
  DeleteFile(Dir + 'rscomm.dll');
  DeleteFile(Dir + 'plcommpro.dll');
  DeleteFile(Dir + 'plcomms.dll');
  DeleteFile(Dir + 'plrscagent.dll');
  DeleteFile(Dir + 'plrscomm.dll');
  DeleteFile(Dir + 'pltcpcomm.dll');
  DeleteFile(Dir + 'plusbcomm.dll');
  DeleteFile(Dir + 'plcommutils.dll');
  DeleteFile(Dir + 'ZKCommuCryptoClient.dll');
  DeleteFile(Dir + 'ZKEMCrypto.dll');
  DeleteFile(Dir + 'libareacode.dll');
  DeleteFile(Dir + 'IOTCAPIs.dll');
  DeleteFile(Dir + 'RDTAPIs.dll');
  DeleteFile(Dir + 'p4p.dll');
  DeleteFile(Dir + 'p4pcomm.dll');
end;
...
begin
  ...
  begin
    ...
    if FileExists(AddBackslash(SysDir) + 'zkemkeeper.dll') or FileExists(ExpandConstant('{app}\dll\zkemkeeper.dll')) then
    begin
      Res := MsgBox(
        '是否注销并删除系统目录 (' + SysDir + ') 中的 ZKTeco COM SDK DLL 组件？' + #13#10 + #13#10 +
        '注销并删除后，系统中的 zkemkeeper COM 组件将失效。' + #13#10 +
        '若此机器上有其他软件正在使用该 DLL，建议选择“否”保留组件。' + #13#10 + #13#10 +
        '是否仍要注销并删除组件？',
        mbConfirmation,
      ...
      if Res = IDYES then
      begin
        Regsvr32Path := AddBackslash(SysDir) + 'regsvr32.exe';
        if FileExists(AddBackslash(SysDir) + 'zkemkeeper.dll') then
        begin
          Exec(Regsvr32Path, '/u /s "' + AddBackslash(SysDir) + 'zkemkeeper.dll"', SysDir, SW_HIDE, ewWaitUntilTerminated, ResultCode);
        end;

        DeleteSdkDlls(SysDir);
      end;
    end;
  end;
end;
